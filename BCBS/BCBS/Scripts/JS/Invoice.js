$(document).ready(function () {
    ...
    $("#CustomerId").change(function () {
        if ($(this).val() > 0) {
            ...
            debugger;
            if ($("#ContractId").val() != "" && $("#ContractId").val() != null && $("#CustomerId").val() > 0) {
        ...
        columns: [
            ...
            //},
            {
             ...
             },
             {
                 field: "ProjectName",
                 title: "Project Name",
                 //template: "#if(FeesType == 'Transaction'){ #<span>#=ProjectName# -  #=Service # ( #=Volume # claims @ $#=Rate # ) <span># } else { #<span>#=ProjectName# - #=Service# ( #=Volume # claims @ #=FeesType#) <span> # }#",
                 template: '#:changeTemplate(FeesType,ProjectName,Service,Rate,Volume)#',
                 //template: "#if(FeesType == 'Transaction'){#= ProjectName # - #=Service #  ( #=Volume #Claims @ $#=Rate # )}",
             },
             //{
             //    field: "Service",
             //    title: "Service",
...
function GetActivitiesDetail() {
    var source = new kendo.data.DataSource({
        ...
        transport: {
            read: {
                type: "GET",
                url: baseURL + "/Customer/GetActivitiesByContactId",
...
function bindContractDDLbyCustomerId(customerid) {
    $.ajax({
        url: baseURL + "/Customer/GetContractsByCustomerIdHaveActivity",
        data: { Id: customerid },
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        async: false,
        success: function (resultdata) {
            ...
            $("#ContractId").multiselect('selectAll', false);
            $("#ContractId").multiselect('updateButtonText');
