    ...
    $("#FeesType").change(function () {
        ...
        if ($(this).val() != "") {
            if ($(this).val() == "Transaction") {
                $(".volume").show();
                //$("#Ammount").attr("readonly", "readonly");
...
function SetFeesTypeDetail(value) {
    ...
    //});
    var label = "Amount($)";
    if (feestypedata[0].FeesType == "Transaction") {
        label = "Rate/Transaction($)";
    }
    else if (feestypedata[0].FeesType == "Hourly") {
        label = "Hourly Rate($)"
    }
